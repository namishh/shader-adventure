...
float randomStain(vec2 uv, float seed) {
    vec2 pos = vec2(random(vec2(seed, seed * 0.1)), random(vec2(seed * 0.2, seed * 0.3)));
...
void main() {
    ...
    float cornerDist = min(min(length(uv), length(uv - vec2(1.0, 0.0))), min(length(uv - vec2(0.0, 1.0)), length(uv - vec2(1.0, 1.0))));
    ...
    if(isLand) {
        ...
        if(forestRegion && terrainValue > 0.55) {
            for(int i = 0; i < 100; i++) {
                ...
                vec2 forestPos = vec2(random(vec2(seed, seed * 0.13)) * 0.1 + uv.x - 0.05, random(vec2(seed * 0.27, seed * 0.31)) * 0.1 + uv.y - 0.05);
        ...
        if(mountainRegion) {
            ...
            for(int i = 0; i < 80; i++) {
                ...
                vec2 mountainPos = vec2(random(vec2(seed, seed * 0.17)) * 0.1 + uv.x - 0.05, random(vec2(seed * 0.29, seed * 0.37)) * 0.1 + uv.y - 0.05);
